...
def classify():
    ...
    txt=CTkLabel(master=frame,text=prediction_text,font=("Fantasy",40),pady=5,padx=5)
    txt.pack(anchor="s",expand=True,pady=3,padx=3)



button_to_select = CTkButton(master=app, text = "Select an Image", fg_color = "red", command = selectfile)
button_to_select.pack(padx = 5, pady = 5)
button_to_select.place(relx = 0.4, rely = 0.9,anchor="center")

classify_button=CTkButton(master=app,text="What is It?!!",fg_color="green",command=classify)
